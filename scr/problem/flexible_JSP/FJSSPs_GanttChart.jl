...
function FJSSPs_Gantt(jobs, machines, processing_times, machine_assignments, start_times; 
                         title="Job Shop Scheduling Gantt Chart", size=(800, 400))
    """
    Plots a professional Gantt chart for JSSP results using bar! function.
    ...
    for op in keys(start_times)
        job, op_num = op
        machine = machine_assignments[op]
        start = start_times[op]
        duration = processing_times[(op[1], op[2], machine)]
        ...
        annotate!(plt, 
            ...
            text("J$(job)O$(op_num)", 8, :white, :center))
    end
    ...
    completion_times = [start_times[op] + processing_times[(op[1], op[2], machine_assignments[op])] for op in keys(start_times)]
